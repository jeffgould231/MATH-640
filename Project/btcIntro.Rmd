...
```{r eval = F}
tictoc::tic()
for(Date in as.character(seq.Date(from = as.Date("2016-07-01"), 
                       to = as.Date("2021-04-14"),
                       by = "1 day"))){
  temp_data <- rgdax::public_candles(product_id = "BTC-USD",
                                     start = glue::glue("{Date}T00:00:00.000000Z"),
                                     end = glue::glue("{as.Date(Date) + lubridate::days(1)}T00:00:00.000000Z"),
                                     granularity = 300)
  if(Date == "2016-07-01"){
  ...
  Sys.sleep(.5) ## Pause is required so we don't max out API requests/5s periods
